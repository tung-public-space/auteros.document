...
class LeafoScssCompiler {
	...
	protected $formatter = "LeafoScssFormatterNested";
	...
	public function compile($code, $name = null)
	{
		...
		$this->parser = new LeafoScssParser($name);
	...
	protected function injectVariables(array $args)
	{
		...
		$parser = new LeafoScssParser(__METHOD__, false);
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
			...
			$parser = new LeafoScssParser($path, false);
...
class LeafoScssParser {
	...
	static protected function makeOperatorStr($operators) {
		return '('.implode('|', array_map(array('LeafoScssParser','preg_quote'),
			$operators)).')';
	}
	 ...
	 *
	 * Consider the function LeafoScssCompiler::keyword(). (All parse functions are
	 * structured the same.)
	 *
	 ...
	 *
	 * All of these parse functions are powered by LeafoScssCompiler::match(), which behaves
	 ...
	 * avoid this, LeafoScssCompiler::seek() is used to remember and set buffer positions.
	 *
	...
	protected function selectorSingle(&$out) {
		...
		while (true) {
			...
			// self
			if ($this->literal("&", false)) {
				$parts[] = LeafoScssCompiler::$selfSelector;
				continue;
			}
...
/**
 * LeafoScssFormatterNested
 *
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class LeafoScssFormatterNested extends LeafoScssFormatter {
	public $close = " }";
...
/**
 * LeafoScssFormatterCompressed
 *
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class LeafoScssFormatterCompressed extends LeafoScssFormatter {
	public $open = "{";
	public $tagSeparator = ",";
	public $assignSeparator = ":";
	public $break = "";
